"""
Прогон router agent на датасете. Сохраняет CSV с предсказаниями и метаданными.
"""
import argparse
import asyncio
import time
...
from tests.common.prelude import build_llm, read_jsonl, write_results_csv
...
async def _run_one(agent: RouterAgent, item: dict) -> dict:
    thread_id = f"router-test-{item['id']}-{uuid.uuid4().hex[:8]}"
    t0 = time.perf_counter()
    error = ""
    response: dict = {}
    metadata: dict = {}
    try:
        result = await agent.process_user_message(item["raw_input"], thread_id)
    ...
    return {
        ...
        "predicted_routed_to": response.get("routed_to") or "",
        "predicted_document_type": response.get("document_type") or "",
